...
START_TEST(test_map_init_fail) {
    map *m = map_init(SIZE_MAX, SIZE_MAX);
    ck_assert(m == NULL);

    m = map_init(SIZE_MAX / 1000, SIZE_MAX / 1000);
    ck_assert(m == NULL);
}
END_TEST
...
START_TEST(test_map_length) {
    float value = 3.5;
    map *m = map_init(sizeof(float), sizeof(float));
...
Suite* suite_map() {
    ...
    tcase_add_test(test_case, test_map_init_fail);
